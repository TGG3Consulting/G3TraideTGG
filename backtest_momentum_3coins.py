...
coins = [
    ...
    ('SOL', r'G:\BinanceFriend\SOLUSDT_volatility_2y_with_LS.xlsx'),
]

for symbol, filepath in coins:
...
for threshold in [0, 3, 4, 5, 10]:
    c = {'trades': 0, 'wins': 0, 'pnl': 0, 'max_dd': 0, 'trades_2m': 0, 'wins_2m': 0, 'pnl_2m': 0}
    for sym, _ in coins:
        r = [x for x in results_all if x['symbol'] == sym and x['threshold'] == threshold][0]
        c['trades'] += r['trades']
        c['wins'] += r['wins']
        c['pnl'] += r['pnl']
        c['max_dd'] = max(c['max_dd'], r['max_dd'])
        c['trades_2m'] += r['trades_2m']
        c['wins_2m'] += r['wins_2m']
        c['pnl_2m'] += r['pnl_2m']
...
print()
print("=" * 85)
print("BY COIN (5% threshold)")
print("=" * 85)
print()
print("Coin      Trades   Win%        PnL      MaxDD | 2M Tr 2M Win%    2M PnL")
print("-" * 75)

for sym, _ in coins:
    r = [x for x in results_all if x['symbol'] == sym and x['threshold'] == 5][0]
    win_pct = r['wins']/r['trades']*100 if r['trades'] > 0 else 0
    win_2m = r['wins_2m']/r['trades_2m']*100 if r['trades_2m'] > 0 else 0
    print(f"{sym:<10} {r['trades']:>6}  {win_pct:>5.1f}%  ${r['pnl']:>+9,.0f}  ${r['max_dd']:>7,.0f} | {r['trades_2m']:>5} {win_2m:>5.1f}%  ${r['pnl_2m']:>+7,.0f}")
