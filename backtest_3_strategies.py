# -*- coding: utf-8 -*-
"""
Backtest 3 strategies with priority: MOMENTUM > MOMENTUM_LS > LS_FADE

Rules:
- One position at a time
- If multiple signals, use priority
- Next signal only after position closes (next day)
"""
import pandas as pd
import numpy as np
...
ls_confirm = 0.60      # MOMENTUM_LS: crowd < 60%
ls_extreme = 0.65      # LS_FADE: crowd >= 65%
lookback = 7
...
def get_signal(signal_day):
    """
    Check signals in priority order: MOMENTUM > MOMENTUM_LS > LS_FADE
    ...
    if pd.isna(pc) or pd.isna(lp) or pd.isna(sp):
        return None, None

    # 1. MOMENTUM (priority 1)
    if pc >= momentum_threshold:
        return 'MOMENTUM', 'LONG'
    if pc <= -momentum_threshold:
        return 'MOMENTUM', 'SHORT'

    # 2. MOMENTUM_LS (priority 2)
...
strategy_counts = {'MOMENTUM': 0, 'MOMENTUM_LS': 0, 'LS_FADE': 0}
...
# ==========================================
print('=' * 70)
print('  3 STRATEGIES: MOMENTUM > MOMENTUM_LS > LS_FADE')
...
# Per strategy
for strat in ['MOMENTUM', 'MOMENTUM_LS', 'LS_FADE']:
...
# Save
result_df.to_excel(r'G:\BinanceFriend\backtest_3_strategies.xlsx', index=False)
print()
print('Saved: backtest_3_strategies.xlsx')
