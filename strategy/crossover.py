"""Crossover detection for moving average strategies.
...
class CrossoverDetector:
    ...
    def update(
        ...
        if self._prev_fast is not None and self._prev_slow is not None:
            ...
            if result != CrossoverType.NONE:
                logger.debug(
                    "Crossover detected",
                    type=result.value,
                    fast_prev=float(self._prev_fast),
                    fast_curr=float(fast_ma),
                    slow_prev=float(self._prev_slow),
                    slow_curr=float(slow_ma),
                )
